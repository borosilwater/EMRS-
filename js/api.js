...
class EMRApi {
    ...
    async loginTeacher(loginId, password, remember = false) {
        const response = await this.makeRequest(API_CONFIG.ENDPOINTS.LOGIN_TEACHER, {
            method: 'POST',
            body: { 
                loginId, // Can be email or employee ID
                password, 
                remember 
            }
        });
...
const ApiUtils = {
    ...
    // Validate email or employee ID
    validateTeacherLoginId(loginId) {
        // Check if it's an email
        if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(loginId)) {
            return true;
        }
        // Check if it's an employee ID (alphanumeric, min 6 chars)
        if (/^[a-zA-Z0-9]{6,}$/.test(loginId)) {
            return true;
        }
        return false;
    }
